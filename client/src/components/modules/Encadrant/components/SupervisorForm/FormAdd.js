...
import {addSupervisor} from '../../../../../actions/supervisor/addSupervisorAction';
import { required, maxLength,  number, email, renderInput }
...
const FormAdd = ({...props}) => {
    ...
    return (
        <Container fluid className='nv-main-content mb-5 mt-5'>
            <Form onSubmit={handleSubmit((data) => props.addSupervisor(data, props.history.push('/encadrants')) )} className="mt-5 p-5 p-3">
                <Legend desc='Formulaire pour ajouter un nouveau encadrant de stage'>{t('newSupervisor')}</Legend>
               <p className='font-weight-normal'><i className="fa fa-user"/> {t('personalInformation')}</p>
                ...
                <Col lg={{size: 6, offset: 0}} md={{size: 12, offset: 0}} sm={{size: 12, offset: 0}} xs={{size: 12, offset: 0}}>
                    <FormGroup className="mb-1">
                        <Label id='element' for="prenom" sm={10}> {t('lastName')}</Label>
                        <Field
                            autoComplete="off"
                            type="text"
                            name="lastname"
                ...
                <Col>
                    <FormGroup check row>
                        <Button
                            type='submit'>
                            <i className='mr-2 fas fa-user-tie'/>&nbsp; {t('btnSaveSupervisor')}
                        </Button>
                    </FormGroup>
                </Col>
            </Form>
...
const mapStateToProps = ({loadAddSupervisor}) => {
    return {
        loadAddSupervisor,
    }
};

export default connect(mapStateToProps, {addSupervisor})(reduxForm({
    form: 'FormAdd'
})(FormAdd));
